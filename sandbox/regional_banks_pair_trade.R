...
regional <- financial_sector[grep(pattern="Regional", x=financial_sector[,"Industry"]),]

# Filter to assets that are the investable universe
regional <- regional[regional[,"Ticker"] %in% betas[,"Symbol"],]

symbols <- as.character(regional[,"Ticker"])
...
for(i in 1:length(symbols)){
  ...
  while(j <= length(symbols)){
    if(j != i){
      ...
      # alignSymbols(c(symbols[i], symbols[j]))
      sym1 <- get(symbols[i])
      sym1 <- Cl(sym1)
...
sharpe.ratios <- unlist(lapply(table.ret, function(x) x[3,]))
annualized.return <- unlist(lapply(table.ret, function(x) x[1,]))
